...
\begin{document}
	...
	\begin{enumerate}
		\item $\varphi(r)$ --- безразмерный положительный потенциал.
		\begin{itemize}
			\item $\varphi(r) > 0$
		...
		\end{itemize}
		\item $r$ --- безразмерный радиус
		\item $v$ --- безразмерная скорость:
		\begin{equation*}
			v = \frac{V}{v_{esc}}
		\end{equation*}

		где $V$ --- настоящая скорость, $v_{esc}$ --- вторая космическая 
		скорость на поверхности небесного тела.
		\begin{itemize}
			\item $v_{esc} = \sqrt{2GM/R}$, $G$ --- гравитационная постоянная, 
			$M$ --- масса небесного тела, $R$ --- радиус небесного тела
		\end{itemize}

		\item $e$ --- положительная безразмерная энергия определяется как
		\begin{equation*}
			e = \varphi(r) - v^2
		\end{equation*}
		\item $L$ --- безразмерный момент импульса
		\begin{equation*}
			l = \frac{L_{\text{размерный}}}{R v_{esc}} = rv\sin{\theta}
		\end{equation*}
		\item $L_m(e)$ --- максимальный момент импульса при данной энергии, 
		$r_m(e)$ --- точка достижения этого максимума.
		\begin{equation}
			L^2_m(e) = \max_r{r^2(\varphi(r)-e)}
			\label{eq:l_m_e}
		\end{equation}
		\item $x_l = \sqrt{1 - \cfrac{l^2}{l_m^2}}$
		\item $T(e,L)$ --- время, за которое частица пролетает от $r_{min}$ до $r_{max}$.
		\begin{equation*}
		...
		\begin{itemize}
			...
			\item $T(e,l)$ разделяется на 2 части: 
	внутренняя часть $T_{in}$, когда $r < 1$ и внешняя часть $T_{out}$, когда $r > 1$. 
		\end{itemize}
